...
local volume = create_button(volume_symbol, volume_unmuted_color, x.trans, x.color1)
...
local microphone = create_button(microphone_symbol, microphone_unmuted_color, x.trans, x.color1)
...
local panel_icon = wibox.widget {
    markup = helpers.colorize_text("", x.color5),
    font = beautiful.ifont .. "22",
    widget = wibox.widget.textbox
}
panel_icon:connect_signal("mouse::enter", function()
    panel_pop.visible = true
end)

local mpdPop = require("candy.music-pop")
local music = wibox.widget {
    {
        awful.widget.watch("mpc current -f '   %artist% - %title%'", 1, function(widget, stdout)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mywibox = awful.wibar({position = "top", screen = s, ontop = true, visible = true, bg = x.color0})
